...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($date) || empty($size) || empty($shape) || empty($color) || empty($type) || empty($weight) || empty($origin) 
     || empty($amount) || empty($image) || empty($certificate) || empty($visibility)) {
        $errorMessage = "All information are required.";
    } else {
        // Construct the SQL query
        $sql = "INSERT INTO inventory (date, size, shape, colour, type, weight, origin, amount, image,  certificate, visibility) 
                VALUES ('$date', '$size', '$shape', '$color', '$type', '$weight', '$origin', '$amount', '$image', '$certificate', '$visibility')";
