...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Basic validation
    if ($biddingStoneId && $startingBid && $noOfCycles && $cycleDuration && $startDate && $finishDate) {
        ...
        ");
        $stmt->bind_param("disssi", $startingBid, $noOfCycles, $cycleDuration, $startDate, $finishDate, $biddingStoneId);

        if ($stmt->execute()) {
            // Redirect back with success
            header("Location: upcomingBid.php?biddingStone_id=" . $biddingStoneId);
            exit();
        } else {
