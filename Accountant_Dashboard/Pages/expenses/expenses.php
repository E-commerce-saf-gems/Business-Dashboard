...
$sql = "SELECT t.transaction_id, t.date, 'Sales' as type, c.email AS email, t.amount , CONCAT(st.colour,' ',st.type,' ',st.size,' ',' carats') AS stone
        FROM transactions as t
        JOIN customer as c ON t.customer_id = c.customer_id
        JOIN inventory as st ON t.stone_id = st.stone_id
        UNION ALL
        SELECT p.payment_id, p.date, 'Purchase' as type, b.email AS email, p.amount, CONCAT(st.colour,' ',st.type,' ',st.size,' ',' carats') AS stone
        FROM payments as p
